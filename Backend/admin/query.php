...
if(isset($_POST['register'])) {
    if(isset($_GET['id'])){
        $data_id = $_GET['id'];
        $assigned = $_POST['year'];
        if($assigned == 'Dr. Bindu Thakral'){
            $insert = "INSERT INTO `Dr.Bindu` (`id`, `name`, `email`, `rollno`, `mobile_no`, `year`, `program`, `issue`, `brief`, `date`) select * from `query` where id = '$data_id'";
        ...
        else if($assigned=='Mr. Kshitij Gupta'){
            $insert = "INSERT INTO `Kshitij` (`id`, `name`, `email`, `rollno`, `mobile_no`, `year`, `program`, `issue`, `brief`, `date`) select * from `query` where id = '$data_id'";
            $result = mysqli_query($con,$insert);
            if($result){
                echo"<script>alert('Succesfully inserted')</script>";
            }
            ...
            $insert = "INSERT INTO `Sadgi` (`id`, `name`, `email`, `rollno`, `mobile_no`, `year`, `program`, `issue`, `brief`, `date`) select * from `query` where id = '$data_id'";
            $result = mysqli_query($con,$insert);
            if($result){
                echo"<script>alert('Succesfully inserted')</script>";
            }
